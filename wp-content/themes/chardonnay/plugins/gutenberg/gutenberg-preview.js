...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			chardonnay_create_observer( 'chardonnay-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				chardonnay_gutenberg_editor_init();
			} );
		} else {
		 	chardonnay_gutenberg_editor_init();
		}
	...
	function chardonnay_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'chardonnay_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'chardonnay_inited' ) ) {
	...
	// Init on page load
	function chardonnay_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = chardonnay_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + CHARDONNAY_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + CHARDONNAY_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in CHARDONNAY_STORAGE['override_classes'] ) {
				$obj.addClass( CHARDONNAY_STORAGE['override_classes'][i].replace( '%s', CHARDONNAY_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( chardonnay_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && CHARDONNAY_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'chardonnay_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			chardonnay_remove_observer( 'chardonnay-check-editor-styles-wrapper' );
			chardonnay_create_observer( 'chardonnay-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'chardonnay_inited' );
	}
} );
