...
jQuery( document ).ready(
	function() {
		...
		chardonnay_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="chardonnay_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.chardonnay_post_editor' );
						if ( link.length > 0 ) {
							chardonnay_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.chardonnay_post_editor', function(e) {
						chardonnay_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('chardonnay_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function chardonnay_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
