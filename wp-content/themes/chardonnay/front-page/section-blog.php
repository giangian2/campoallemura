<div class="front_page_section front_page_section_blog<?php
	$chardonnay_scheme = chardonnay_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $chardonnay_scheme ) && ! chardonnay_is_inherit( $chardonnay_scheme ) ) {
		echo ' scheme_' . esc_attr( $chardonnay_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( chardonnay_get_theme_option( 'front_page_blog_paddings' ) );
	if ( chardonnay_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$chardonnay_css      = '';
		$chardonnay_bg_image = chardonnay_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$chardonnay_anchor_icon = chardonnay_get_theme_option( 'front_page_blog_anchor_icon' );
	$chardonnay_anchor_text = chardonnay_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( chardonnay_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$chardonnay_css      = '';
			$chardonnay_bg_mask  = chardonnay_get_theme_option( 'front_page_blog_bg_mask' );
			$chardonnay_bg_color_type = chardonnay_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $chardonnay_bg_color_type ) {
				$chardonnay_bg_color = chardonnay_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$chardonnay_caption = chardonnay_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $chardonnay_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $chardonnay_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $chardonnay_caption, 'chardonnay_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$chardonnay_description = chardonnay_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $chardonnay_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $chardonnay_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $chardonnay_description ), 'chardonnay_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! chardonnay_exists_trx_addons() ) {
						...
						chardonnay_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>
